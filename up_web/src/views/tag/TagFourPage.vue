<template>
  ...
  <div class="tag-list">
    <div class="tag-list-item" v-for="data in datas" :key="data.tag.id">
      ...
      <div class="item-info">
        <div><i class="el-icon-warning-outline" style="color:#409eff"></i></div>
        <div>{{data.tag.business}}</div>
      </div>
      <div class="item-option">
        <div>
          <el-button  @click="startTag(data, 1)" class="item-option-btn" v-if="data.state == 1" type="success" size="mini">{{data.tag.id}}启动</el-button>
          <el-button  @click="startTag(data, 0)" class="item-option-btn" v-if="data.state == 0" type="warning" size="mini">{{data.tag.id}}停止</el-button>
        </div>
...
<el-dialog title="新建四级标签" :visible.sync="dialogFormVisible">
    <el-form ref="modelTag" :model="modelTag" :rules="rules">
...
export default {
  ...
  methods: {
    ...
    createTag: function(event) {
      ...
      axios
      .post(config.baseApi + "tags/model", this.modelTag)
      .then(response => {
